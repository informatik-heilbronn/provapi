...
class StartPage(tk.Frame):
    ...
    def start_comparison(self):
        ...
        if self.count == 0:
            ...
            path = camera.take_photo("tank")
            key = dmcr.get_dmc_from_file(path, "tank")
            self.dmc_tank = key
            ...
            path = camera.take_photo("leitung")
            key = dmcr.get_dmc_from_file(path, "bauschein")
...
if __name__ == "__main__":
    thread = threading.Thread(target=camera.main_camera1, args=())
    thread.start()
    provapi = ProvaPiApp()

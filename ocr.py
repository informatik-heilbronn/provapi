...
def readImage(file_path):
    ...
    while counter < 12 and not found:
        ...
        if len(list) > 0:
            for tank in keys.get_tank_keys():
                if SequenceMatcher(None, list[0], tank).ratio() > 0.7 and tank[-1] == list[0][-1]:
                    for leitung in keys.get_leitung_keys():
                        if SequenceMatcher(None, list[1], leitung).ratio() > 0.7 and leitung[-1] == list[1][-1]:
                            found = True
    ...
    print(string)
    if found:
        codes = str(list[0])[len(list[0]) - 1], str(list[1])[len(list[1]) - 1]
        print("successful - Codes: " + codes[0] + " + " + codes[1])
